...
const openQrModal=async()=>{QRM.mode="seguimiento";QRM.scope="global";$("#tkQrModal").hidden=false;document.body.classList.add("modal-open");await qrLoadEditor();qrPaintEditor()};
...
const qrCollect=()=>{$$(".tk-qr-row").forEach((row,i)=>{const id=row.dataset.id||"",t=row.querySelector('[data-k="titulo"]')?.value?.trim()||`Respuesta ${i+1}`,x=qrCanon(row.querySelector('[data-k="texto"]')?.value?.trim()||"");QRM.rows[i]={...QRM.rows[i],id,titulo:t,texto:x,orden:i+1,activo:true}});QRM.rows=qrSeedRows(QRM.rows).slice(0,10)};
