...
const openContactPanel=()=>openDialog("#tkContactOverlay",{initialFocus:"#tkContactClose",onCloseRequest:closeContactPanel});
const closeContactPanel=()=>closeDialog("#tkContactOverlay");
...
const openQrModal=async()=>{if(!canManageQuickReplies("global"))return denyQuickReplyManagement();QRM.mode="seguimiento";QRM.scope="global";openDialog("#tkQrModal",{initialFocus:"#tkQrClose",onEscape:()=>{const p=$("#tkQrVarsPop");if(p&&!p.classList.contains("hidden")){p.classList.add("hidden");$("#tkQrVarsBtn")?.setAttribute("aria-expanded","false");return false}return true},onCloseRequest:qrClose});await qrLoadEditor();qrPaintEditor()};
const qrClose=()=>closeDialog("#tkQrModal");
...
const closeSystemPanel=()=>closeDialog("#tkSystemOverlay");
...
const cleanupUploaded=async rows=>{for(const row of rows||[]){let ok=true;if(row.attachment_id){const r=await s.functions.invoke("support-submit-secure",{body:{attachment_id:row.attachment_id},headers:{"x-media-operation":"delete"}});if(r.error||!r.data?.deleted)ok=false}uploadStage("CLEANUP",{file:row.file,operation:"rollback_current_upload",result:ok?"ok":"error"});if(!ok)console.error("ORPHAN_CLEANUP_FAILED",{stage:"CLEANUP",bucket:"soporte_adjuntos",operation:"rollback_current_upload"})}};
const uploadPublicLogFiles=async()=>{if(!ST.logFiles.length)return[];const out=[],rollback=[];try{for(const file of ST.logFiles){const row={file,attachment_id:"",storage_path:""};uploadStage("VALIDATION",{file,operation:"server_validate",result:"start"});const bytes=await file.arrayBuffer(),digest=await crypto.subtle.digest("SHA-256",bytes),checksum=[...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,"0")).join(""),body=new FormData();body.append("ticket_id",ID);body.append("visibilidad","publica");body.append("file",file);const up=await s.functions.invoke("support-submit-secure",{body,headers:{"x-media-operation":"upload","idempotency-key":crypto.randomUUID(),"x-content-sha256":checksum}});if(up.error||!up.data?.attachment_id)throw uploadFailure("MEDIA_EDGE",up.error||{code:up.data?.code||"MEDIA_UPLOAD_FAILED"});row.attachment_id=up.data.attachment_id;row.storage_path=up.data.storage_path;uploadStage("MEDIA_EDGE",{file,operation:"canonical_upload",result:up.data.replayed?"replayed":"ok"});rollback.push(row);out.push({id:row.attachment_id,nombre:file.name,tipo:file.type||null,peso:file.size,storage_path:row.storage_path,url:null,origen:"soporte_interno",checksum_sha256:up.data.checksum_sha256||checksum})}}catch(error){await cleanupUploaded(rollback);throw error}Object.defineProperty(out,"_rollbackRows",{value:rollback,enumerable:false});return out};
...
const renderLogs=()=>{
  ...
  (LOGS||[]).forEach((raw,idx)=>{
    ...
    const isSystem=x.side==="sys";
    entries.push({
      ts,
      order:100+idx,
      html:`<div class="log-msg ${isSystem?"sys":x.side==="me"?"me":"other"} ${x.kind||""}" data-event-id="${esc(raw?.id||"")}"${isSystem?` data-system-event-type="${esc(x.systemType||"unknown")}"`:""}><div class="log-meta"><b>${esc(isSystem?x.title:logSenderName(x))}</b><span>${esc(x.time)}</span></div><div class="log-text">${esc(x.text)}</div></div>`
    });
  });
...
LOGS=mergeTicketEventHistory({legacy,normalized:evNorm});HEAT.rows=Array.isArray(heatRows?.data)?heatRows.data:[];
