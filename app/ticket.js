...
const load=async()=>{const auth=await guardSession();if(!auth)return;const profile=(await getProfile())||{rol:"soporte"};ST.profile=profile||null;ensureAppShell({page:"ticket",title:"",kicker:"",role:profile?.rol||"soporte",actionsHtml:""});setAppRole(profile.rol||"soporte");syncQuickReplyAdminUi();registerInternalSearchProvider({sb:s,user:auth.user,rol:profile?.rol||"soporte"});if(!ID){toast("Falta ID del ticket","bad");setTimeout(()=>location.href="tickets.html",900);return}const tk=await loadTicketCore();if(!tk){toast("Ticket no encontrado","bad");setTimeout(()=>location.href="tickets.html",900);return}T=tk;await loadTicketContext();await hydrateTicketUi()};
