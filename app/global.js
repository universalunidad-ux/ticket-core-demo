...
const appBrandHtml=()=>`<a class="app-brand" href="dashboard.html" aria-label="Inicio"><img src="../IMG/janome.jpg" alt=""></a>`;
...
const initAppHeader=(page)=>{
  ...
  document.querySelectorAll("[data-nav]").forEach(x=>{x.classList.remove("is-active");x.removeAttribute("aria-current")});
  if(page)document.querySelectorAll(`[data-nav="${page}"]`).forEach(x=>{x.classList.add("is-active");x.setAttribute("aria-current","page")});
  markNavVisited(page);
  renderNavAttention();
};
...
export const registerGlobalSearchProvider=provider=>{if(!provider||typeof(provider.search||provider)!=="function")throw new TypeError("Global search provider inválido");__globalSearchProvider=typeof provider==="function"?{id:"anonymous",search:provider}:provider;__globalSearchSeq++;__globalSearchAbort?.abort();__globalSearchAbort=null;return()=>{if(__globalSearchProvider===provider||__globalSearchProvider?.search===provider){clearGlobalSearchProvider()}}};
export const clearGlobalSearchProvider=()=>{__globalSearchProvider=null;__globalSearchSeq++;__globalSearchAbort?.abort();__globalSearchAbort=null;__globalSearchItems=[];closeGlobalSuggest()};
export const getGlobalSearchProvider=()=>__globalSearchProvider;
export const setGlobalSearchData=({clientes=[],tickets=[],extras=[]}={})=>{__globalSearchData={clientes,tickets,extras:[...GLOBAL_PAGES,...extras]};if(!__globalSearchProvider||__globalSearchProvider.id==="page-data")registerGlobalSearchProvider({id:"page-data",search:async q=>globalSuggestRowsFromData(q,__globalSearchData)});if(document.activeElement?.id==="globalSearchInput")runGlobalSearch()};
...
const renderGlobalSuggest=items=>{const box=$("#globalSearchSuggest"),input=$("#globalSearchInput"),clear=$("#globalSearchClear"),q=input?.value||"",x=norm(q);if(!box||!input)return;if(clear)clear.hidden=!q;if(!q)return closeGlobalSuggest();if(x.length<2&&!items.length)return globalSearchState("Escribe al menos 2 caracteres.");const groups=["Tickets","Clientes","Secciones"],html=groups.map(group=>{const rows=items.filter(x=>x.group===group);return rows.length?`<section class="app-search-group"><div class="app-search-group-title">${group}</div>${rows.map(row=>{const idx=items.indexOf(row),id=`globalSearchOption${idx}`;return`<a id="${id}" class="item suggest-item" role="option" aria-selected="false" data-search-index="${idx}" href="${esc(row.href)}"><div class="item-title">${esc(row.label)}</div><div class="item-meta">${esc(row.sub||"")}</div></a>`}).join("")}</section>`:""}).join("");box.innerHTML=html||'<div class="app-search-state">Sin resultados.</div>';box.classList.remove("hidden");box.removeAttribute("hidden");input.setAttribute("aria-expanded","true");__globalSearchIndex=-1};
const runGlobalSearch=async()=>{const input=$("#globalSearchInput"),q=input?.value||"",x=norm(q);if(!input||!q)return closeGlobalSuggest();if(x.length<2)return globalSearchState("Escribe al menos 2 caracteres.");const provider=__globalSearchProvider;if(!provider)return globalSearchState("Búsqueda no disponible en esta vista.");const seq=++__globalSearchSeq;__globalSearchAbort?.abort();const controller=new AbortController();__globalSearchAbort=controller;globalSearchState("Buscando…");try{const remote=await provider.search(q,{signal:controller.signal,limit:12});if(seq!==__globalSearchSeq||controller.signal.aborted)return;const sections=globalSuggestRowsFromData(q,{clientes:[],tickets:[],extras:GLOBAL_PAGES});__globalSearchItems=[...new Map([...(Array.isArray(remote)?remote:[]),...sections].filter(Boolean).map(row=>[row.k||`${row.type}_${row.href}`,row])).values()].slice(0,12);renderGlobalSuggest(__globalSearchItems)}catch(err){if(seq!==__globalSearchSeq||controller.signal.aborted)return;console.warn("GLOBAL_SEARCH_PROVIDER_ERROR",err?.message||"search_failed");globalSearchState("No se pudo buscar. Inténtalo de nuevo.")}};
