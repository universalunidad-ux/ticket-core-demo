...
export const setRailOpenCount=count=>{const role=document.querySelector("#appShell")?.dataset.appRole||"soporte",key=`tc_canonical_ticket_count_${role}`;let n=count==null?NaN:Number(count);if(!Number.isFinite(n)){try{const cached=sessionStorage.getItem(key);n=cached==null?NaN:Number(cached)}catch{n=NaN}}else try{sessionStorage.setItem(key,String(n))}catch{};$$("#railOpenCount,#railOpenCountMob").forEach(el=>{if(Number.isFinite(n)&&n>=0){el.textContent=String(n);el.hidden=false}else{el.textContent="—";el.hidden=true}})};
