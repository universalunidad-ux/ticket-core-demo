...
{titulo:"Pedir modelo",texto:"Para revisar {producto}, por favor comparta el modelo exacto de la máquina y, si lo tiene, el número de serie."},
{titulo:"Pedir foto/video",texto:"Por favor envíe una foto clara y un video corto, de máximo 20 segundos, donde se vea {producto}, el problema y la acción que realiza."},
{titulo:"Pedir garantía",texto:"Para validar la garantía de {producto}, por favor comparta el comprobante de compra, la fecha, el modelo y el número de serie."},
{titulo:"Pedir muestra",texto:"Para revisar {producto}, por favor comparta una foto de la muestra, el tipo de tela, hilo, aguja y ajuste de tensión."},
...
const TK_LIST_COLUMNS=["abierto","en_proceso","resuelto"];
const tkDashboardReferrer=()=>{try{return new URL(document.referrer).origin===location.origin&&/\/dashboard\.html$/.test(new URL(document.referrer).pathname)}catch{return false}};
const tkEnsureDashboardOrigin=()=>{const url=new URL(location.href);if(!url.searchParams.has("from")&&tkDashboardReferrer()){url.searchParams.set("from","dashboard");history.replaceState(history.state,"",url)}return url.searchParams.get("from")==="dashboard"};
const tkKpiIsCanonical=(kpi=qp("kpi"))=>kpi==="urgent"?FILTER.priority==="urgente":kpi==="waiting"?FILTER.state==="esperando_cliente":kpi==="urgent_stale"?!!FILTER.urgentStale:kpi==="resolved"?FILTER.state==="resuelto":kpi==="first_response_overdue"?!!FILTER.frBreach:kpi==="sla_overdue"?!!FILTER.rsBreach:false;
...
const syncFilterUI=()=>{$("#tkSearch")&&($("#tkSearch").value=FILTER.q||"");$("#tkFilterPriority")&&($("#tkFilterPriority").value=FILTER.priority||"");$("#tkFilterState")&&($("#tkFilterState").value=FILTER.state||"");$("#tkFilterType")&&($("#tkFilterType").value=FILTER.type||"");$("#tkFilterClient")&&($("#tkFilterClient").value=FILTER.client||"");["tkNoEvidence","tkImpactHigh","tkUrgentStale","tkNoClientLinked","tkMatchMedium","tkFrBreach","tkRsBreach","tkSlaSoon"].forEach(id=>$("#"+id)?.classList.toggle("btn-brand",id==="tkNoEvidence"?FILTER.noEvidence:id==="tkReadyClose"?FILTER.readyClose:id==="tkImpactHigh"?FILTER.impactHigh:id==="tkUrgentStale"?FILTER.urgentStale:id==="tkNoClientLinked"?FILTER.noClientLinked:id==="tkMatchMedium"?FILTER.matchMedium:id==="tkFrBreach"?FILTER.frBreach:id==="tkRsBreach"?FILTER.rsBreach:FILTER.slaSoon));$("#tkOnlyUrgent")?.classList.toggle("btn-brand",FILTER.priority==="urgente");tkSyncFilterActiveUi()};
const applyUrlFilters=()=>{resetFilters();const view=qp("view"),state=qp("state"),priority=qp("priority"),clienteId=qp("cliente_id"),type=qp("type"),q=qp("q"),noClient=qp("noClient"),match=qp("match"),kpi=qp("kpi");if(q)FILTER.q=q;if(view==="open")FILTER.state="";if(view==="waiting_client")FILTER.state="esperando_cliente";if(view==="urgent")FILTER.priority="urgente";if(state)FILTER.state=state;if(priority)FILTER.priority=priority;if(type)FILTER.type=type;if(clienteId){FILTER.clienteId=String(clienteId);const t=TK.find(x=>String(x?.cliente_id)===String(clienteId));FILTER.client=t?.clientes?.nombre||""}if(noClient==="1")FILTER.noClientLinked=true;if(match==="medio")FILTER.matchMedium=true;if(qp("noEvidence")==="1")FILTER.noEvidence=true;if(qp("impactHigh")==="1")FILTER.impactHigh=true;if(qp("urgentStale")==="1")FILTER.urgentStale=true;if(qp("firstResponseOverdue")==="1")FILTER.frBreach=true;if(qp("slaOverdue")==="1")FILTER.rsBreach=true;if(qp("slaSoon")==="1")FILTER.slaSoon=true;if(kpi==="urgent")FILTER.priority="urgente";if(kpi==="waiting")FILTER.state="esperando_cliente";if(kpi==="urgent_stale")FILTER.urgentStale=true;if(kpi==="resolved")FILTER.state="resuelto";if(kpi==="first_response_overdue")FILTER.frBreach=true;if(kpi==="sla_overdue")FILTER.rsBreach=true;tkRestoreListPosition();syncFilterUI();tkSyncNavigationContext()};
...
const board=()=>$("#tkBoard");
const openTicketDetail=(id,qk="")=>{const ticket=byId(id),pos=tkCurrentListPosition(ticket);tkSyncListUrl({column:pos.column,page:pos.page});const detail=new URL("ticket.html",location.href);detail.searchParams.set("id",String(id));if(qk)detail.searchParams.set("qk",qk);detail.searchParams.set("return",location.pathname.replace(/[^/]+$/,"")+"tickets.html"+location.search);location.href=detail.href};
...
const closeQuickEditor=()=>{tkQrDelCancel();closeDialog("#tkQrEditor")};
...
const openQuickEditor=async()=>{if(!TK_AUTH_CTX?.isAdmin)return toast("Las respuestas Globales y de Producto son administradas por Administrador.","warn");mountQuickEditor();openDialog("#tkQrEditor",{initialFocus:"#tkQrClose",onEscape:()=>{if(QRB_DEL_IDX<0)return true;tkQrDelCancel();return false},onCloseRequest:closeQuickEditor});await qrBoardLoad(QRB.scope||"global")};
...
const tkColModalRow=t=>{
  ...
  return`<div class="tk-col-modal-row${id&&id===String(SELECTED_ID)?" is-selected":""}" data-id="${htmlAttr(id)}" data-open-ticket="${htmlAttr(id)}" role="link" tabindex="0" aria-label="Abrir ticket ${folio}: ${titulo}">
...
const mobileStateSet=(k,{historyMode="push"}={})=>{
  ...
  const page=Math.max(0,Number(COL_PAGE[next])||0);
  if(page)url.searchParams.set("page",String(page+1));else url.searchParams.delete("page");
  url.searchParams.set("layout","kanban");
...
const renderPager=(key,total)=>{const pages=pageCount(total),page=(COL_PAGE[key]||0)+1,info=$("#info-"+key);if(info){info.textContent=`${page}/${pages}`;info.setAttribute("aria-label",`Página ${page} de ${pages}`)}$("#prev-"+key)&&($("#prev-"+key).disabled=(COL_PAGE[key]||0)<=0);$("#next-"+key)&&($("#next-"+key).disabled=(COL_PAGE[key]||0)>=pages-1);const pager=$("#pager-"+key);if(pager)pager.classList.toggle("hidden",pages<=1)};
...
const renderCompactPager=rows=>{const pages=compactPageCount(rows),page=(COMPACT_PAGE||0)+1;$("#tkCompactPager")&&($("#tkCompactPager").innerHTML=`<div class="cp-total">Total: ${rows.length} ticket${rows.length===1?"":"s"}</div><nav class="cp-nav" aria-label="Paginación de la lista de tickets"><button class="cp-btn" id="tkCompactPrev" type="button" aria-label="Página anterior" title="Página anterior" ${(COMPACT_PAGE||0)<=0?"disabled":""}>‹</button><div class="cp-page" aria-live="polite" aria-label="Página ${rows.length?page:0} de ${rows.length?pages:0}">${rows.length?page:0}/${rows.length?pages:0}</div><button class="cp-btn" id="tkCompactNext" type="button" aria-label="Página siguiente" title="Página siguiente" ${!rows.length||(COMPACT_PAGE||0)>=pages-1?"disabled":""}>›</button></nav>`)};
...
const closedRow=t=>{const ev=evidenceCount(t),id=ticketDomId(t.id),cliente=htmlText(fmtCardText(t.empresa_capturada||t.clientes?.nombre||"Sin registro")),folio=htmlText(t.folio||"—"),titulo=htmlText(t.titulo||"Sin título"),cre=closedMiniDate(t.fecha_creacion||t.created_at),cer=closedMiniDate(t.fecha_cierre||t.fecha_actualizacion),desde=String(t.desde_cuando||"").trim(),cambio=String(t.ultimo_cambio||""),dias=daysSince(t.fecha_actualizacion||t.fecha_creacion),ctx=[desde?`Desde: ${desde}`:"",cambio&&cambio!=="sin_cambio"&&cambio!=="no_se"?"Cambio previo":"",dias===0?"Hoy":`Sin avance ${dias}d`].filter(Boolean);return`<div class="closed-row" data-id="${htmlAttr(id)}" role="link" tabindex="0" aria-label="Abrir ticket ${folio}: ${titulo}"><div class="closed-priority"><span class="tag ${ticketPriorityCls(t.prioridad)}">${htmlText(t.prioridad||"media")}</span></div><div class="closed-client"><strong>${cliente}</strong><span>${folio}</span></div><div class="closed-case"><strong>${titulo}${ev?` <span class="closed-clip">📎</span>`:""}</strong><span>${htmlText(String(t.descripcion||"Sin descripción").slice(0,120))}</span></div><div class="closed-context">${ctx.map(x=>`<span>${htmlText(x)}</span>`).join("")}</div><div class="closed-dates"><div class="closed-datebox"><small>Creado</small><strong>${htmlText(cre.day)}</strong><span>${htmlText(cre.mon)}</span></div><div class="closed-datebox is-close"><small>Cerrado</small><strong>${htmlText(cer.day)}</strong><span>${htmlText(cer.mon)}</span></div></div></div>`};
...
const openClosedModal=()=>{closeTicketFloaters?.();document.body.classList.remove("tk-toolbar-popup-open");$("#tkToolbarPopupBackdrop")?.classList.remove("open");$("#tkToggleClosed")?.setAttribute("aria-expanded","true");const m=$("#tkClosedModal");if(!m)return toast("Falta #tkClosedModal en tickets.html.","bad");openDialog(m,{initialFocus:"#tkClosedQ",fallbackFocus:"#tkMoreFiltersBtn",onCloseRequest:closeClosedModal});renderClosed()};
// #tkToggleClosed vive dentro de #tkAdvancedFilters, que closeTicketFloaters() oculta al abrir el
// modal: trigger y fallback quedarian invisibles y el foco caeria a body. #tkMoreFiltersBtn es el
// ancla persistente de este callsite. El trigger real sigue teniendo prioridad si continua visible.
const closeClosedModal=()=>{$("#tkToggleClosed")?.setAttribute("aria-expanded","false");closeDialog("#tkClosedModal",{fallbackFocus:"#tkMoreFiltersBtn"})};
...
const fetchTicketsRest=async(signal)=>{const url=s.supabaseUrl,h=await restHeaders(),ctx=DEV_READONLY()?{isAdmin:true,canAllScope:true,userId:""}:await tkAuthContext(),q=new URLSearchParams({select:"*",order:"fecha_actualizacion.desc"});/* TC-U15A-1: alcance canónico aplicado EN LA CONSULTA. Soporte queda forzado a "mine"
   aunque manipule ?scope=; sólo administración accede a all/unassigned. */const scope=resolveTicketScope(qp("scope"),{isAdmin:!!ctx.canAllScope}),assignedFilter=scopeAssignedFilter(scope,ctx.userId);if(assignedFilter)q.set("asignado_a",assignedFilter);else if(ctx.canAllScope&&qp("assignee"))q.set("asignado_a",`eq.${qp("assignee")}`);const endpoint=`${url}/rest/v1/tickets?${q}`;console.info("TICKETS_REQUEST",{endpoint:new URL(endpoint).pathname+new URL(endpoint).search,scope,role:ctx.canAllScope?"admin":"support"});const r=await fetch(endpoint,{headers:h,signal});if(!r.ok){const body=await r.text();const e=new Error(`Tickets HTTP ${r.status}: ${body}`);e.status=r.status;e.endpoint=endpoint;throw e}const rows=await r.json(),ids=[...new Set(rows.map(x=>x.cliente_id).filter(Boolean))];if(!ids.length){rows.forEach(t=>t.clientes=null);return rows}const clientEndpoint=`${url}/rest/v1/clientes?select=id,nombre&id=in.(${ids.join(",")})`;const cr=await fetch(clientEndpoint,{headers:h,signal});if(!cr.ok){const body=await cr.text();const e=new Error(`Clientes de tickets HTTP ${cr.status}: ${body}`);e.status=cr.status;e.endpoint=clientEndpoint;throw e}const clientes=await cr.json(),map=Object.fromEntries(clientes.map(x=>[x.id,x]));rows.forEach(t=>t.clientes=map[t.cliente_id]||null);return rows};
...
const fetchTicketsWithRecovery=request=>resolveTicketLoad({
  request:()=>fetchTicketsRest(request.signal),
  withTimeout,
...
const tkLoadJanomeSeedRows=async(signal)=>{
  ...
  try{
    const r=await fetch("janome-test-tickets.json?v="+Date.now(),{cache:"no-store",signal});
    if(r.ok){
...
const tkMergeJanomeVisualSeed=async(base=[],signal)=>{
  const clean=Array.isArray(base)?base:[];
  if(!tkJanomeSeedEnabled())return clean;
  const seed=await tkLoadJanomeSeedRows(signal);
  if(!seed.length){
...
const renderMetrics=()=>{
  ...
  setTxt("mWait",waitingTicketRows(rows.map(x=>({...x,estado:ticketStateKey(rawState(x))}))).length);
...
const closeNewTicketModal=()=>{closeDialog("#tkModal",{fallbackFocus:"#tkNewBtn"});closeTicketFloaters?.()};
...
const forceCloseNewTicketModal=()=>{const m=$("#tkModal");if(m)m.style.display="";closeNewTicketModal()};
...
const bindNewTicketClose=()=>{const m=$("#tkModal");if(!m)return;["tkModalCloseX","tkClose","tkCancel"].forEach(id=>$("#"+id)&&($("#"+id).onclick=e=>{e.preventDefault();e.stopImmediatePropagation?.();forceCloseNewTicketModal()}));m.onclick=null;if(m.__tkOutsideClose)m.removeEventListener("click",m.__tkOutsideClose,true);m.__tkOutsideClose=e=>{const inside=e.target?.closest?.("#tkModal .modal,#tkModal .tk-new-modal,.modal,.tk-new-modal");if(e.target===m||!inside){e.preventDefault();e.stopImmediatePropagation?.();forceCloseNewTicketModal()}};m.addEventListener("click",m.__tkOutsideClose,true)};
...
const openNewTicketModal=()=>{closeTicketFloaters?.();const m=$("#tkModal");if(m)m.style.display="grid";prepareNewTicketModal?.();bindNewTicketClose?.();openDialog(m,{initialFocus:"#tkTitulo",fallbackFocus:"#tkNewBtn",onCloseRequest:forceCloseNewTicketModal});setTimeout(mountNewTicketSendInsideDesc,0)};
...
const toggleView=()=>{if(isMobileTickets()){VIEW="kanban";localStorage.setItem("tc_tickets_view","kanban");$("#tkBoard")?.classList.remove("hidden");$("#tkCompact")?.classList.add("hidden");tkSyncListUrl();renderAll();return}VIEW=VIEW==="kanban"?"compact":"kanban";$("#tkBoard")?.classList.toggle("hidden",VIEW==="compact");$("#tkCompact")?.classList.toggle("hidden",VIEW!=="compact");document.querySelector(".board-layout")?.classList.remove("hidden");localStorage.setItem("tc_tickets_view",VIEW);syncHeaderIconButtons();tkSyncListUrl();renderAll();syncSelected();if(QUICK.open)fillQuickPanel(QUICK.lastAction||"modelo")};
const bindKeyboard=()=>{if(document.documentElement.dataset.ticketsKeyboardBound)return;document.documentElement.dataset.ticketsKeyboardBound="1";document.addEventListener("keydown",async e=>{const interactive=e.target?.closest?.("a,button,input,textarea,select,[contenteditable='true'],[role='button'],[role='link']"),t=selectedTicket();if(interactive||e.metaKey||e.ctrlKey||e.altKey||!t)return;if(e.key==="Enter"){e.preventDefault();return openTicketDetail(t.id,QUICK.open?(QUICK.lastAction||""):"")}if(e.key==="e"||e.key==="E"){e.preventDefault();return await moveTicket(t.id,"en_proceso")}if(e.key==="w"||e.key==="W"){e.preventDefault();return await moveTicket(t.id,"esperando_cliente")}if(e.key==="r"||e.key==="R"){e.preventDefault();return await moveTicket(t.id,"resuelto")}if(e.key==="q"||e.key==="Q"){e.preventDefault();QUICK.lastAction=QUICK.lastAction||"modelo";return fillQuickPanel(QUICK.lastAction)}if(e.key==="x"||e.key==="X"){e.preventDefault();return await runQuickCopy(t.id,"modelo")}if(e.key==="p"||e.key==="P"){e.preventDefault();return await runQuickCopy(t.id,"evidencia")}if((e.key==="c"||e.key==="C")&&ticketStateKey(rawState(t))==="resuelto"){e.preventDefault();return await closeTicket(t.id)}})};
const bindTicketRowKeyboard=()=>{if(document.documentElement.dataset.tkRowKeyboardBound)return;document.documentElement.dataset.tkRowKeyboardBound="1";document.addEventListener("keydown",e=>{if(!["Enter"," "].includes(e.key))return;const row=e.target?.closest?.(".k-card,.compact-row,.closed-row,.tk-col-modal-row[data-open-ticket]");if(!row||e.target!==row)return;const id=row.dataset.openTicket||row.dataset.id;if(!id)return;e.preventDefault();e.stopPropagation();SELECTED_ID=String(id);syncSelected();openTicketDetail(id)},true)};
bindTicketRowKeyboard();
...
const clearAllFilters=()=>{resetFilters();const url=new URL(location.href);["kpi","view","q","priority","state","type","cliente_id","noClient","match","noEvidence","impactHigh","urgentStale","firstResponseOverdue","slaOverdue","slaSoon","assignee"].forEach(k=>url.searchParams.delete(k));history.replaceState(history.state,"",url);tkResetPages();tkSyncListUrl({resetPage:true});syncFilterUI();ensureSelectedVisible();renderAll();syncSelected();syncHeaderClearBtns();closeTicketMenus()};

const setSelectVal=(id,v)=>{const el=$("#"+id);if(el)el.value=v||""};
const syncKpiUrl=(kind,active)=>{const key={urgent:"urgent",wait:"waiting",stale:"urgent_stale",solved:"resolved",fr:"first_response_overdue",rs:"sla_overdue"}[kind],url=new URL(location.href);url.searchParams.delete("view");if(key&&active)url.searchParams.set("kpi",key);else url.searchParams.delete("kpi");history.pushState(history.state,"",url)};
...
const colPageClick=(id)=>{const m=String(id||"").match(/^(prev|next)-(.+)$/);if(!m)return false;const dir=m[1],key=m[2];if(!Object.prototype.hasOwnProperty.call(COL_PAGE,key))return false;COL_PAGE[key]=dir==="prev"?Math.max(0,(COL_PAGE[key]||0)-1):(COL_PAGE[key]||0)+1;tkSyncListUrl({mode:"push",column:key,page:COL_PAGE[key]});renderAll();requestAnimationFrame(()=>$("#"+id)?.focus());return true};
