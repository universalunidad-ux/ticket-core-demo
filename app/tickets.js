...
const tkData=v=>{
  ...
  return /^[A-Za-z0-9._:-]{1,128}$/.test(x)?x:"";
};
let LOAD_SEQ=0;
...
const tkSyncFilterActiveUi=()=>{
  try{
    ...
    console.warn("TK_FILTER_ACTIVE_SYNC_FALLBACK");
  }
};
...
const board=()=>$("#tkBoard");
const openTicketDetail=(id,qk="")=>{
  if(DEV_READONLY())return toast("El detalle está deshabilitado en la demo segura.","warn");
  location.href=`ticket.html?id=${encodeURIComponent(id)}${qk?`&qk=${encodeURIComponent(qk)}`:""}`;
};
...
const tkQuickProductSaveFinal=(t,rows)=>{
  ...
  }catch(e){console.warn("TK_PRODUCT_QR_SAVE_LOCAL_WARN")}
};
...
const renderBoardQuickDb=async(ticket=null)=>{
  ...
  try{
    ...
    console.warn("TK_QUICK_ROWS_FALLBACK");
...
const tkWireQuickScopeTabsV2=()=>{
  ...
  document.addEventListener("click",e=>{
    ...
    }).catch(err=>console.warn("TK_QUICK_SCOPE_RENDER_ERROR"));
  },true);
};
...
const tkColModalRow=t=>{
  const ev=evidenceCount(t),
    ...
    id=tkData(t.id),
    cliente=tkEsc(fmtCardText(t.empresa_capturada||t.clientes?.nombre||"Sin registro")),
    folio=tkEsc(t.folio||"—"),
    prioridad=tkEsc(t.prioridad||"media"),
    ...
    titulo=tkEsc(fmtCardText(t.titulo||"Sin título")),
    desc=tkEsc(fmtCardText((t.descripcion||"Sin descripción").slice(0,118))),
    acts=state==="abierto"
      ?`<button class="mini btn-ghost is-disabled" type="button" disabled>←</button><button class="mini btn-ghost" type="button" data-ticket-state="${tkAttr(id)}|en_proceso">→</button>`
      :state==="en_proceso"||state==="esperando_cliente"
        ?`<button class="mini btn-ghost" type="button" data-ticket-state="${tkAttr(id)}|abierto">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${tkAttr(id)}|resuelto">→</button>`
        :state==="resuelto"
          ?`<button class="mini btn-ghost" type="button" data-ticket-state="${tkAttr(id)}|en_proceso">←</button><button class="mini btn-ok" type="button" data-ticket-close="${tkAttr(id)}">✓</button>`
          :`<button class="mini btn-ghost" type="button" data-ticket-state="${tkAttr(id)}|en_proceso">↺</button>`;
  return`<div class="tk-col-modal-row${String(t.id)===String(SELECTED_ID)?" is-selected":""}" data-id="${tkAttr(id)}" data-open-ticket="${tkAttr(id)}">
    <div class="closed-priority"><span class="tag ${ticketPriorityCls(t.prioridad)}">${prioridad}</span></div>
    ...
    <div class="tk-col-modal-acts"><div class="tk-col-arrows">${acts}</div><button class="mini btn-ghost k-action-bolt" type="button" data-quick-panel="${tkAttr(id)}|modelo" title="Acciones rápidas">⚡</button></div>
  </div>`;
};
...
const closedRow=t=>{
  const ev=evidenceCount(t),
    id=tkData(t.id),
    cliente=tkEsc(t.empresa_capturada||t.clientes?.nombre||"Sin registro"),
    folio=tkEsc(t.folio||"—"),
    prioridad=tkEsc(t.prioridad||"media"),
    titulo=tkEsc(t.titulo||"Sin título"),
    descripcion=tkEsc((t.descripcion||"Sin descripción").slice(0,120)),
    cre=closedMiniDate(t.fecha_creacion||t.created_at),
    cer=closedMiniDate(t.fecha_cierre||t.fecha_actualizacion),
    desde=String(t.desde_cuando||"").trim(),
    cambio=String(t.ultimo_cambio||""),
    dias=daysSince(t.fecha_actualizacion||t.fecha_creacion),
    ctx=[
      desde?`Desde: ${desde}`:"",
      cambio&&cambio!=="sin_cambio"&&cambio!=="no_se"?"Cambio previo":"",
      dias===0?"Hoy":`Sin avance ${dias}d`
    ].filter(Boolean);
  return`<div class="closed-row" data-id="${tkAttr(id)}"><div class="closed-priority"><span class="tag ${ticketPriorityCls(t.prioridad)}">${prioridad}</span></div><div class="closed-client"><strong>${cliente}</strong><span>${folio}</span></div><div class="closed-case"><strong>${titulo}${ev?` <span class="closed-clip">📎</span>`:""}</strong><span>${descripcion}</span></div><div class="closed-context">${ctx.map(x=>`<span>${tkEsc(x)}</span>`).join("")}</div><div class="closed-dates"><div class="closed-datebox"><small>Creado</small><strong>${cre.day}</strong><span>${cre.mon}</span></div><div class="closed-datebox is-close"><small>Cerrado</small><strong>${cer.day}</strong><span>${cer.mon}</span></div></div></div>`;
};
...
const DEV_READONLY=()=>location.pathname.startsWith("/ticket-core-demo/")&&new URLSearchParams(location.search).get("readonly")==="1";
...
const fetchTicketsRest=async()=>{const url=s.supabaseUrl,h=await restHeaders(),ctx=await tkAuthContext(),q=new URLSearchParams({select:"*",order:"fecha_actualizacion.desc"});if(!ctx.isAdmin)q.set("asignado_a",`eq.${ctx.userId}`);else if(qp("assignee"))q.set("asignado_a",`eq.${qp("assignee")}`);const endpoint=`${url}/rest/v1/tickets?${q}`;const r=await fetch(endpoint,{headers:h});if(!r.ok){await r.text();const e=new Error(`Tickets HTTP ${r.status}`);e.status=r.status;throw e}const rows=await r.json(),ids=[...new Set(rows.map(x=>x.cliente_id).filter(Boolean))];if(!ids.length){rows.forEach(t=>t.clientes=null);return rows}const clientEndpoint=`${url}/rest/v1/clientes?select=id,nombre&id=in.(${ids.join(",")})`;const cr=await fetch(clientEndpoint,{headers:h});if(!cr.ok){await cr.text();const e=new Error(`Clientes de tickets HTTP ${cr.status}`);e.status=cr.status;throw e}const clientes=await cr.json(),map=Object.fromEntries(clientes.map(x=>[x.id,x]));rows.forEach(t=>t.clientes=map[t.cliente_id]||null);return rows};
const updateTicketRest=async(id,payload)=>{if(!id)throw new Error("Falta id de ticket");const h=await restHeaders(),r=await fetch(`${s.supabaseUrl}/rest/v1/tickets?id=eq.${encodeURIComponent(id)}`,{method:"PATCH",headers:{...h,Prefer:"return=representation"},body:JSON.stringify(payload)});const txt=await r.text();if(!r.ok)throw new Error(`Ticket HTTP `);return JSON.parse(txt||"[]")};
...
const tkLoadJanomeSeedRows=async()=>{
  ...
  try{
    ...
    console.warn("JANOME_SEED_JSON_UNAVAILABLE");
  }
  const seen=new Set();
...
const card=t=>{
  const crit=isCritical(t),
    ...
    meta=ticketMeta(t),
    titulo=tkEsc(fmtCardText(t.titulo||"Sin título")),
    descRaw=fmtCardText(t.descripcion||"Sin descripción"),
    desc=tkEsc(descRaw.length>128?descRaw.slice(0,128).trim()+"…":descRaw),
    prioridad=tkEsc(t.prioridad||"media"),
    cls=`k-card ${crit?"is-critical":""} ${staleCls(t)} ${String(t.id)===String(SELECTED_ID)?"is-selected":""}`.trim(),
    acts=state==="abierto"
      ?`<button class="mini btn-ghost is-disabled" type="button" disabled title="Sin estado anterior">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso" title="Mover a en proceso">→</button>`
      :state==="en_proceso"
        ?`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|abierto" title="Regresar a abierto">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|resuelto" title="Marcar como resuelto">→</button>`
        :state==="esperando_cliente"
          ?`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso" title="Regresar a en proceso">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|resuelto" title="Marcar como resuelto">→</button>`
          :state==="resuelto"
            ?`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso" title="Reabrir">←</button><button class="mini btn-ok" type="button" data-ticket-close="${safeId}" title="Doble clic para cerrar">✓</button>`
            :`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso" title="Reabrir">↺</button>`;
  return`<article class="${cls}" draggable="${EDIT_MODE?"true":"false"}" data-id="${safeId}"><div class="k-head"><div class="k-title-row"><div class="k-title-main"><div class="k-title-line"><div class="k-title">${titulo}</div><span class="tag ${ticketPriorityCls(t.prioridad)}">${prioridad}</span></div><div class="k-company-line"><span class="k-company">${cliente}</span></div></div>${ev?`<span class="k-clip" title="${ev} adjunto${ev>1?"s":""}" aria-label="${ev} adjunto${ev>1?"s":""}">📎</span>`:""}</div></div><div class="k-desc">${desc}</div>${meta.length?`<div class="k-submeta">${meta.slice(0,3).map(x=>`<span class="mini-meta">${tkEsc(x)}</span>`).join("")}</div>`:""}<div class="actions quick-inline k-card-actions">${acts}<button class="mini btn-ghost k-action-bolt" type="button" data-quick-panel="${safeId}|modelo" title="Acciones rápidas">⚡</button></div></article>`;
};
...
const compactRow=t=>{
  const crit=isCritical(t),
    ...
    titulo=tkEsc(fmtCardText(t.titulo||"Sin título")),
    descRaw=fmtCardText(t.descripcion||"Sin descripción"),
    desc=tkEsc(descRaw.length>118?descRaw.slice(0,118).trim()+"…":descRaw),
    prioridad=tkEsc(t.prioridad||"media"),
    acts=state==="abierto"
      ?`<button class="mini btn-ghost is-disabled" type="button" disabled>←</button><button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso">→</button>`
      :state==="en_proceso"
        ?`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|abierto">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|resuelto">→</button>`
        :state==="esperando_cliente"
          ?`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|resuelto">→</button>`
          :state==="resuelto"
            ?`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso">←</button><button class="mini btn-ok" type="button" data-ticket-close="${safeId}">✓</button>`
            :`<button class="mini btn-ghost" type="button" data-ticket-state="${safeId}|en_proceso">↺</button>`;
  return`<div class="compact-row ${crit?"is-critical":""} ${String(t.id)===String(SELECTED_ID)?"is-selected":""}" data-id="${safeId}"><div class="compact-case"><div class="compact-topline"><strong class="compact-client">${cliente}</strong><span class="tag ${ticketPriorityCls(t.prioridad)}">${prioridad}</span>${ev?`<span class="compact-clip" title="${ev} adjunto${ev>1?"s":""}">📎</span>`:""}</div><div class="compact-title">${titulo}</div><span class="compact-desc">${desc}</span></div><div class="compact-context">${meta.map(x=>`<span>${tkEsc(x)}</span>`).join("")}</div><div class="compact-actions">${acts}<button class="mini btn-ghost k-action-bolt" type="button" data-quick-panel="${safeId}|modelo" title="Acciones rápidas">⚡</button></div></div>`;
};
...
const openGearMenuHard=()=>{
  ...
  try{
    ...
    console.warn("TK_GEAR_PREP_WARN");
  }
...
const moveTicket=async(id,next)=>{
  ...
  try{
    if(!isUuid(id)){}
      ...
      if(s.supabaseUrl&&s.supabaseKey)await updateTicketRest(id,patch);
...
const sendQuickReply=async()=>{
  ...
  try{
    if(DEV_READONLY?.()){
      if(s.supabaseUrl&&s.supabaseKey&&updateTicketRest)await updateTicketRest(t.id,patch);
      ...
      if(ev.error)console.warn("TK_QUICK_EVENT_WARN");
    ...
    console.error("TK_QUICK_SEND_ERROR");
...
const tkPlayTestSound2K=()=>{
  ...
  try{
    ...
    console.warn("TK_SOUND_2K_ERROR");
    tkStopTestSound2K();
  }
};
...
const tkNotifSync2K=()=>{
  try{
    ...
    console.warn("TK_NOTIF_SYNC_WARN");
  }
};
...
const tkPlayTestSound=()=>{
  try{
    ...
    console.warn("TK_TEST_SOUND_FALLBACK");
...
const bootTickets=()=>{if(document.documentElement.dataset.tkBooted)return;document.documentElement.dataset.tkBooted="1";console.info("tickets boot");try{bindStatic()}catch(e){console.error("bindStatic error")}try{bindDynamic()}catch(e){console.error("bindDynamic error")}load().then(()=>{console.info("tickets loaded",TK.length);try{bindDynamic()}catch(e){console.error("bindDynamic after load error")}if(typeof syncHeroMetrics==="function")syncHeroMetrics()}).catch(err=>{console.error("load error");toast(msg(err),"bad")});setTimeout(()=>{try{bindDynamic()}catch(e){console.error("bindDynamic timeout 600 error")}},600);setTimeout(()=>{try{bindDynamic()}catch(e){console.error("bindDynamic timeout 1600 error")}},1600);setInterval(()=>{if(QUICK.open||!$("#tkModal")?.hidden||!$("#tkClosedModal")?.hidden||!$("#tkGearMenu")?.hidden||!$("#tkAdvancedFilters")?.hidden)return;load().then(()=>{try{bindDynamic()}catch(e){console.error("bindDynamic interval error")}if(typeof syncHeroMetrics==="function")syncHeroMetrics()}).catch(e=>console.warn("poll load error"))},60000)};
