...
const openClosedModal=()=>{closeTicketFloaters?.();document.body.classList.remove("tk-toolbar-popup-open");$("#tkToolbarPopupBackdrop")?.classList.remove("open");$("#tkToggleClosed")?.setAttribute("aria-expanded","true");const m=$("#tkClosedModal");if(!m)return toast("Falta #tkClosedModal en tickets.html.","bad");openDialog(m,{initialFocus:"#tkClosedQ",fallbackFocus:"#tkToggleClosed",onCloseRequest:closeClosedModal});renderClosed()};
const closeClosedModal=()=>{$("#tkToggleClosed")?.setAttribute("aria-expanded","false");closeDialog("#tkClosedModal",{fallbackFocus:"#tkToggleClosed"})};
