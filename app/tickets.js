...
const compactRow=t=>{const crit=isCritical(t),ev=evidenceCount(t),id=ticketDomId(t.id),state=ticketStateKey(rawState(t)),cliente=htmlText(fmtCardText(t.empresa_capturada||t.clientes?.nombre||"Sin registro")),meta=ticketMeta(t),titulo=htmlText(fmtCardText(t.titulo||"Sin título")),desc=htmlText(fmtCardText(t.descripcion||"Sin descripción")),acts=ticketActions(id,state);return`<div class="compact-row ${crit?"is-critical":""} ${id&&id===String(SELECTED_ID)?"is-selected":""}" data-id="${htmlAttr(id)}"><div class="compact-case"><div class="compact-topline"><strong class="compact-client">${cliente}</strong><span class="tag ${ticketPriorityCls(t.prioridad)}">${htmlText(t.prioridad||"media")}</span>${ev?`<span class="compact-clip" title="${ev} adjunto${ev>1?"s":""}">📎</span>`:""}</div><div class="compact-title">${titulo}</div><span class="compact-desc">${desc.length>118?desc.slice(0,118).trim()+"…":desc}</span></div><div class="compact-context">${meta.map(x=>`<span>${htmlText(x)}</span>`).join("")}</div><div class="compact-actions">${acts}${id?`<button class="mini btn-ghost k-action-bolt" type="button" data-quick-panel="${htmlAttr(id)}|modelo" title="Acciones rápidas">⚡</button>`:""}</div></div>`};
