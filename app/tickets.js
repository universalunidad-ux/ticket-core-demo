...
const filtered=()=>TK.filter(t=>(!FILTER.q||ticketBlob(t).includes(norm(FILTER.q)))&&(!FILTER.priority||norm(t.prioridad)===norm(FILTER.priority))&&(!FILTER.state||ticketStateKey(rawState(t))===ticketStateKey(FILTER.state))&&(!FILTER.type||normTipo(t.tipo)===norm(FILTER.type))&&(!FILTER.clienteId||String(t?.cliente_id)===String(FILTER.clienteId))&&(!FILTER.client||norm(t.clientes?.nombre||t.empresa_capturada||"").includes(norm(FILTER.client)))&&(!FILTER.noEvidence||evidenceCount(t)===0)&&(!FILTER.impactHigh||impactKey(t)==="alta")&&(!FILTER.urgentStale||(["urgente","alta"].includes(norm(t.prioridad))&&!["resuelto","cerrado"].includes(ticketStateKey(rawState(t)))&&new Date(t.fecha_actualizacion||t.fecha_creacion).toDateString()!==new Date().toDateString()))&&(!FILTER.noClientLinked||noClientLinked(t))&&(!FILTER.matchMedium||matchLevel(t)==="medio")&&(!FILTER.frBreach||slaFrBreached(t))&&(!FILTER.rsBreach||slaRsBreached(t))&&(!FILTER.slaSoon||slaSoon(t)));
...
const quickActionText=(t,k)=>{
  const empresa=t?.empresa_capturada||t?.clientes?.nombre||"su empresa";
  const sis=t?.sistema||t?.sistema_detectado||"el producto";
  if(k==="modelo")return`Para revisar ${sis} en ${empresa}, ¿podría compartir el modelo exacto de la máquina y, si lo tiene, el número de serie?`;
  ...
  if(k==="horario")return`¿Podría compartir uno o dos horarios disponibles para revisar ${sis} en ${empresa}, así como el medio preferido de contacto?`;
...
const tkProductDefaults=t=>[
  {titulo:"Pedir modelo",texto:"Para revisar {sistema} en {empresa}, por favor compártenos modelo exacto de la máquina y número de serie si lo tienes."},
  {titulo:"Pedir foto/video",texto:"Por favor envía una foto clara y un video corto máximo de 20 segundos donde se vea {sistema}, el problema y la acción que estás realizando."},
  ...
  {titulo:"Pedir garantía",texto:"Para validar garantía de {sistema}, comparte comprobante de compra, fecha de compra, modelo y número de serie."},
  {titulo:"Confirmar solución",texto:"Se aplicó ajuste / validación operativa en {sistema}. Favor de confirmar si la máquina ya opera correctamente."}
];
...
const qrBoardLoad=async(scope="global")=>{const t=selectedTicket();if(!t)return toast("Selecciona un ticket.","warn");QRB.scope=scope==="producto"?"producto":"global";QRB.mode="seguimiento";try{if(QRB.scope==="producto"){QRB.rows=tkProductRows(t).map((x,i)=>({...x,scope:"producto",modo:QRB.mode,orden:i+1}));while(QRB.rows.length<5)QRB.rows.push({id:"",scope:"producto",modo:QRB.mode,titulo:`Respuesta ${QRB.rows.length+1}`,texto:"",orden:QRB.rows.length+1});qrBoardPaint();$("#tkQrBoardHint")&&($("#tkQrBoardHint").textContent=`Producto: ${t?.sistema||t?.sistema_detectado||"producto"}. Estas respuestas se guardan localmente por ahora.`);return}QRB.rows=await qrLoadScope({ticket:t,scope:"global",modo:QRB.mode,min:7,max:10});qrBoardPaint();$("#tkQrBoardHint")&&($("#tkQrBoardHint").textContent="Globales para todos los tickets.")}catch(e){toast(msg(e),"bad")}};
const openQuickEditor=async()=>{mountQuickEditor();$("#tkQrEditor")?.classList.remove("hidden");await qrBoardLoad(QRB.scope||"global")};
const qrBoardSave=async()=>{const t=selectedTicket();if(!t)return toast("Selecciona un ticket.","warn");qrBoardCollect();try{if(QRB.scope==="producto"){tkQuickProductSaveFinal?.(t,QRB.rows);toast("Respuestas de producto guardadas localmente.","ok");await renderBoardQuickDb(t);closeQuickEditor?.();return}await qrSaveScope({ticket:t,scope:"global",modo:QRB.mode,rows:QRB.rows});toast("Respuestas guardadas.","ok");await renderBoardQuickDb(t);closeQuickEditor?.()}catch(e){toast(msg(e),"bad")}};

const tkQuickScope=()=>QUICK.replyScope==="producto"?"producto":"global";
const tkQuickGlobalFallback=()=>[
  {titulo:"Pedir modelo",texto:"Hola, {usuario}. Para revisar {sistema} en {empresa}, por favor compártenos el modelo exacto de la máquina y, si lo tienes, el número de serie."},
  {titulo:"Pedir foto/video",texto:"Hola, {usuario}. Por favor envía una foto clara y un video corto máximo de 20 segundos donde se vea {sistema}, el problema y la acción que estás realizando."},
  {titulo:"Pedir garantía",texto:"Hola, {usuario}. Para validar garantía de {sistema}, comparte comprobante de compra, fecha de compra, modelo y número de serie."},
  {titulo:"Pedir muestra",texto:"Hola, {usuario}. Para revisar {sistema}, comparte una foto de la muestra de puntada, tipo de tela, hilo usado, aguja instalada y ajuste de tensión."},
  {titulo:"Pedir horario",texto:"Hola, {usuario}. Compártenos por favor uno o dos horarios disponibles para revisar {sistema} en {empresa}, y el medio preferido de contacto para continuar."},
  {titulo:"Confirmar solución",texto:"Hola, {usuario}. Se aplicó ajuste / validación operativa en {sistema}. Favor de confirmar si la máquina ya opera correctamente."},
  {titulo:"Marcar resuelto",texto:"Hola, {usuario}. Se registró solución para {sistema}. El caso queda resuelto y puede reabrirse si el problema vuelve a presentarse."}
];
const tkQuickProductFallback=t=>[
  {titulo:"Puntada irregular",texto:"Hola, {usuario}. Para revisar la puntada en {sistema}, comparte foto de la muestra, tipo de tela, hilo usado, aguja instalada y ajuste de tensión."},
  {titulo:"No cose / no avanza",texto:"Hola, {usuario}. Para revisar {sistema}, confirma si la aguja sube y baja, si los dientes de arrastre se mueven y si el prensatelas está abajo."},
  {titulo:"Enhebrado / tensión",texto:"Hola, {usuario}. Por favor vuelve a enhebrar {sistema} con el prensatelas arriba y comparte foto del recorrido del hilo y del ajuste de tensión."},
  {titulo:"Aguja e hilo",texto:"Hola, {usuario}. Confirma por favor calibre y tipo de aguja, tipo de hilo, tela utilizada y si la aguja es nueva o ya estaba instalada."},
  {titulo:"Accesorio compatible",texto:"Hola, {usuario}. Para validar compatibilidad, comparte modelo exacto de {sistema}, foto del accesorio/refacción y, si aplica, número de parte."},
  {titulo:"Garantía Janome",texto:"Hola, {usuario}. Para validar garantía de {sistema}, comparte comprobante de compra, fecha de compra, modelo, número de serie y video corto del comportamiento."},
  {titulo:"Mantenimiento",texto:"Hola, {usuario}. Para revisar mantenimiento de {sistema}, indica cuándo fue el último servicio y qué síntoma aparece: ruido, atorón, tensión, puntada o avance."}
];
const tkQuickProductKey=t=>String(t?.sistema||t?.sistema_detectado||t?.titulo||t?.id||"producto").trim().toLowerCase().replace(/\s+/g,"_").slice(0,80)||"producto";
...
const tkColModalRow=t=>{
  const ev=evidenceCount(t),
    ...
    cliente=fmtCardText(t.empresa_capturada||t.clientes?.nombre||"Sin registro"),
    folio=t.folio||"—",
    ...
    titulo=fmtCardText(t.titulo||"Sin título"),
    desc=fmtCardText((t.descripcion||"Sin descripción").slice(0,118)),
    acts=state==="abierto"
      ?`<button class="mini btn-ghost is-disabled" type="button" disabled>←</button><button class="mini btn-ghost" type="button" data-ticket-state="${t.id}|en_proceso">→</button>`
      :state==="en_proceso"||state==="esperando_cliente"
        ?`<button class="mini btn-ghost" type="button" data-ticket-state="${t.id}|abierto">←</button><button class="mini btn-ghost" type="button" data-ticket-state="${t.id}|resuelto">→</button>`
        :state==="resuelto"
          ?`<button class="mini btn-ghost" type="button" data-ticket-state="${t.id}|en_proceso">←</button><button class="mini btn-ok" type="button" data-ticket-close="${t.id}">✓</button>`
          :`<button class="mini btn-ghost" type="button" data-ticket-state="${t.id}|en_proceso">↺</button>`;
  return`<div class="tk-col-modal-row${String(t.id)===String(SELECTED_ID)?" is-selected":""}" data-id="${t.id}" data-open-ticket="${t.id}">
    <div class="closed-priority"><span class="tag ${ticketPriorityCls(t.prioridad)}">${t.prioridad||"media"}</span></div>
    ...
    <div class="closed-dates tk-col-dates-pair">
      <div class="closed-datebox"><small>Creado</small><strong>${cre.day}</strong><span>${cre.mon}</span></div>
      <div class="closed-datebox"><small>Actualizado</small><strong>${upd.day}</strong><span>${upd.mon}</span></div>
    </div>
    <div class="tk-col-modal-acts"><div class="tk-col-arrows">${acts}</div><button class="mini btn-ghost k-action-bolt" type="button" data-quick-panel="${t.id}|modelo" title="Acciones rápidas">⚡</button></div>
  </div>`;
};

const mobileStateSet=k=>{
  if (typeof tkSetMobileState === "function") return tkSetMobileState(k);
  MOBILE_STATE = ["abierto","en_proceso","resuelto"].includes(k) ? k : "abierto";
  localStorage.setItem("tc_tickets_mobile_state", MOBILE_STATE);
  renderAll?.();
};
  ...
  document.querySelectorAll("#tkMobileStatebar [data-mobile-state]").forEach(btn=>{
    ...
    if(!btn.dataset.bound){
      ...
      btn.addEventListener("click",e=>{
        e.preventDefault();
        e.stopPropagation();
        mobileStateSet(btn.dataset.mobileState||"abierto");
      });
    }
  });
};
...
const renderMetrics=()=>{
  const rows=tkMetricRows(),today=new Date().toDateString();
  ...
  setTxt("mStale",rows.filter(x=>["urgente","alta"].includes(norm(x.prioridad))&&!["resuelto","cerrado"].includes(ticketStateKey(rawState(x)))&&new Date(x.fecha_actualizacion||x.fecha_creacion).toDateString()!==today).length);
...
const sendQuickReply=async()=>{
  ...
  try{
    if(DEV_READONLY?.()){
      ...
      if(ev.error)console.warn("TK_QUICK_EVENT_WARN",ev.error.message||ev.error);
