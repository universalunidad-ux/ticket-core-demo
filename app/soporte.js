...
const preview=()=>{setTxt("pvName",trimVal("spName")||"—");setTxt("pvSystem",sistemaLabelHuman()||"—");setTxt("pvPhone",(()=>{const t=trimVal("spPhone");if(!t)return"—";return $("#spWhats")?.checked?`${t} · WhatsApp`:t;})());const tt=$("#spTitle")?.value?.trim()||"";setTxt("pvTitle",tt||"—");/* B17C43D: resumen corto en línea, largo envuelve */const row=$("#pvTitle")?.closest(".tk-receipt-row");row?.classList.toggle("tk-receipt-block",tt.length>32)};
...
const send=async e=>{
  ...
  try{
    ...
    stopSubmitFeedback();
    console.error("SUPPORT_SEND_ERROR");
