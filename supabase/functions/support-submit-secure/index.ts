...
const magic_link=`${appUrl}/estado.html?folio=${encodeURIComponent(folio)}&token=${encodeURIComponent(token_publico)}`;
    ...
    return json({ok:true,solicitud_id:solicitud.id,ticket_id:ticket.id,folio,magic_link,magic_expires:token_publico_expira,token_publico,cliente_id,cliente_id_sugerido:match.cliente_id,contacto_id_sugerido:match.contacto_id,match_level:matchNivelEfectivo,match_score:match.score,requires_confirmation:matchNivelEfectivo==="medio"||!!(match.cliente_id&&!empresa_confirmada),requires_consolidation:requiere_consolidacion,suggested_company:match.cliente_nombre,suggested_contact:match.contacto_nombre,status:"ticket_creado"},200);
